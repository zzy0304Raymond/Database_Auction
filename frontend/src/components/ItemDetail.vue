<template>
  <div class="item-detail-container">
    ...
    <div class="recommendations-section">
      ...
      <el-row :gutter="20">
        <!-- 仅在推荐商品存在时渲染 -->
        <el-col :span="6" v-for="recommendation in recommendedItems" :key="recommendation.id" v-if="recommendedItems.length">
          <el-card class="recommendation-item" shadow="hover">
            ...
            <p>Current Bid: ${{ recommendation.currentBid }}</p>
...
export default {
  ...
  created() {
    ...
    this.fetchRecommendations(); // 在组件创建时调用，确保推荐商品被加载
  },
  methods: {
    beforeRouteUpdate(to, from, next) {
      // 如果路由的参数发生变化，重新获取数据
      if (to.params.id !== from.params.id) {
        this.fetchItemDetails(to.params.id); // 重新获取数据
      }
      next();
    },
    ...
    async fetchItemDetails() {
      ...
      try {
        ...
        console.log('Main item details:', response.data); // 输出商品详情数据，检查是否包含 category

        // 确认 category 是否存在
        if (!response.data.category) {
          console.error('Category is missing in the item details.');
        }
    ...
    async fetchRecommendations() {
      try {
        const itemId = this.$route.params.id; // 获取当前商品的 ID
        console.log(`Fetching main item details for ID: ${itemId}`);

        // 1. 通过商品 ID 获取当前商品详细信息
        const itemResponse = await axios.get(`${BACKEND_BASE_URL}/auction-items/${itemId}`);
        this.item = itemResponse.data; // 将当前商品信息存储在 `this.item`
        console.log('Main item details:', this.item);

        // 2. 确认 category 是否存在，如果不存在，抛出错误
        if (!this.item.category) {
          console.error('Category is missing for the main item. Cannot fetch recommendations.');
          return;
        }
        ...
        // 3. 使用当前商品的 category 调用推荐接口，获取推荐商品基本信息
        const recommendationResponse = await axios.get(`${BACKEND_BASE_URL}/auction-items/recommendations`, {
          params: { category }
        });
        
        const  recommendedItems = recommendationResponse.data; // 获取到推荐商品列表
        console.log('Recommended items fetched:', recommendedItems);


          // 排除当前商品本身
        const filteredItems = recommendedItems.filter(item => item.id !== parseInt(itemId));
        console.log('Filtered recommended items (without current item):', filteredItems);

        // 5. 并行获取每个推荐商品的详细信息 (使用过滤后的 filteredItems)
        const detailedItems = await Promise.all(filteredItems.map(async (item) => {
          console.log(`Fetching details for recommended item ID: ${item.id}`);
          const itemDetailResponse = await axios.get(`${BACKEND_BASE_URL}/auction-items/${item.id}`);
          console.log(`Details for recommended item ID: ${item.id}`, itemDetailResponse.data);
          ...
          return {
            ...
            name: item.name,
            price: item.price,
            currentBid: itemDetailResponse.data.currentBid,
            startingBid: itemDetailResponse.data.startingBid,
            description: itemDetailResponse.data.description,
            imageUrl: `data:image/jpg;base64,${itemDetailResponse.data.images}`
          };
        }));

        // 5. 将详细的推荐商品信息存储到 recommendedItems
        this.recommendedItems = detailedItems;
        console.log('Detailed recommended items:', this.recommendedItems);

      } catch (error) {
        console.error('Error fetching recommendations or details:', error);
        this.error = 'Failed to fetch recommended items or their details.';
      }
    },
    ...
    // 查看物品详情
    viewItem(id) {
      console.log('Navigating to item with ID:', id); // 调试
      this.$nextTick(() => {
