...
export default {
  ...
  methods: {
      ...
      onFileChange(e) {
        ...
        console.log("Enter OFC:", file);
        if (file) {
          const reader = new FileReader();
          reader.onload = (event) => {
            const base64String = event.target.result;
            console.log('File reading completed. Base64 string:', base64String);
            // 使用split方法去除逗号之前的内容，只保留纯粹的Base64编码部分
            const pureBase64 = base64String.split(',')[1];
            console.log('Base64 encoded string:', pureBase64);
            console.log("this.editingItem", this.editingItem);
            if (this.editingItem.name) {
              console.log("EEEEEE",this.editingItem.ImageUrl);
              this.editingItem.ImageUrl = pureBase64; // 编辑模式下
            } else {
              console.log('New Item Image URL:', pureBase64);
              console.log("NNNNNNN");
    ...
    addItem() {
      ...
      console.log('Item going to be post:', this.newItem);  // 调试输出
      //这里response改成ItemsID
      // 提交包含Base64图片的表单数据
